...
public class InputHandler extends KeyAdapter
     ...
     */
    void setScale(float sc, int ox, int oy) {
        scale = sc;
        offX = ox;
        offY = oy;
    }
    ...
    private int tx(int x) {
        return scale == 1f ? x : Math.round((x - offX) / scale);
    }

    private int ty(int y) {
        return scale == 1f ? y : Math.round((y - offY) / scale);
    }
    ...
    boolean isUp() {
        return held.contains(settings.upKey);
    }

    boolean isDown() {
        return held.contains(settings.downKey);
    }

    boolean isLeft() {
        return held.contains(settings.leftKey);
    }

    boolean isRight() {
        return held.contains(settings.rightKey);
    }
    ...
    boolean isDash() {
        return held.contains(settings.dashKey);
    }

    boolean isInteract() {
        return held.contains(settings.interactKey);
    }

    boolean isPause() {
        return held.contains(settings.pauseKey);
    }
